...
//	get_moto_measure(&moto_chassis[MotorID], Data);
	...
	uint8_t rx_msg[] = "CAN message received\r\n";
	HAL_UART_Transmit(&huart1, rx_msg, sizeof(rx_msg) - 1, HAL_MAX_DELAY);
...
void set_moto_current(CAN_HandleTypeDef* hcan, s16 iq1, s16 iq2, s16 iq3, s16 iq4)
{
	...
	can_status = HAL_CAN_AddTxMessage(hcan, &CanTx, Data, &Mailbox);
	if (can_status != HAL_OK) {
		uint8_t error_msg[] = "CAN transmission failed\r\n";
		HAL_UART_Transmit(&huart1, error_msg, sizeof(error_msg) - 1, HAL_MAX_DELAY);
	} else {
		uint8_t success_msg[] = "CAN message sent\r\n";
		HAL_UART_Transmit(&huart1, success_msg, sizeof(success_msg) - 1, HAL_MAX_DELAY);
	}

}	

void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *hcan) {
	uint8_t success_msg[] = "CAN message fully sent via Mailbox 0\r\n";
	HAL_UART_Transmit(&huart1, success_msg, sizeof(success_msg) - 1, HAL_MAX_DELAY);
}

void HAL_CAN_TxMailbox1CompleteCallback(CAN_HandleTypeDef *hcan) {
	uint8_t success_msg[] = "CAN message fully sent via Mailbox 1\r\n";
	HAL_UART_Transmit(&huart1, success_msg, sizeof(success_msg) - 1, HAL_MAX_DELAY);
}

void HAL_CAN_TxMailbox2CompleteCallback(CAN_HandleTypeDef *hcan) {
	uint8_t success_msg[] = "CAN message fully sent via Mailbox 2\r\n";
	HAL_UART_Transmit(&huart1, success_msg, sizeof(success_msg) - 1, HAL_MAX_DELAY);
}


void HAL_CAN_ErrorCallback(CAN_HandleTypeDef *hcan) {
	uint8_t error_msg[] = "CAN error occurred\r\n";
	HAL_UART_Transmit(&huart1, error_msg, sizeof(error_msg) - 1, HAL_MAX_DELAY);
}
