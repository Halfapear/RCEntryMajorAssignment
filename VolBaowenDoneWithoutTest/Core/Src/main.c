                ...
                if (i + 3 + length <= len)
                {
                    ...
                    if (checksum == buf[i + 3 + length - 1])
                    {
                        ...
                        if (msgType == 0x01)
                        {
                            ...
                            printf("接收到数据 - 占空比: %lu, 周期: %lu, 频率: %lu\r\n", dutyCycle, cycle, frequency);
                        }
                    }
                    else
                    {
...
void Send_Measurement_Message(void)
{
    ...
    // 通过UART发送报文（使用阻塞方式）
    HAL_UART_Transmit(&huart1, message, index, HAL_MAX_DELAY);
}
