...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	while(huart->Instance==USART3)//如果是串口3
	{
		...
		if (USART3_RX_STA == 0 && USART3_RX_BUF[USART3_RX_STA] != 0xAA) break; //帧头不对，丢掉
		if (USART3_RX_STA == 1 && USART3_RX_BUF[USART3_RX_STA] != 0x55) break; //帧头不对，丢掉
		USART3_RX_STA++;
		if (USART3_RX_STA > USART_REC_LEN) USART3_RX_STA = 0;  ///接收数据错误,重新开始接收

		if (USART3_RX_STA == 25)	//接受完一帧数据
		{
